...
def render():
    ...
    <div class="insight-box coral">
      <b>⚠ {'El problema está claro:' if L == 'es' else 'The problem is clear:'}</b> {'El tráfico crece +12.000%, los ingresos no. La red se estrella. Los operadores invierten la mitad que EE.UU. Alguien tiene que pagar por la red que todos usamos.' if L == 'es' else 'Traffic grows +12,000%, revenue does not. The network is straining. Operators invest half what the US does. Someone has to pay for the network everyone uses.'}
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_before:
        ...
        st.markdown(f'<div style="font-size:0.9rem;color:#CF3B30;font-weight:700;text-transform:uppercase;letter-spacing:1px;">❌ {t("fair_share.baseline", L)}</div>', unsafe_allow_html=True)
        from streamlit_echarts import st_echarts
        r_before = st_echarts(options=_gauge_chart(original_gap_pp, gauge_max, "#CF3B30", ""), height="220px", events={"click": CLICK_FORMAT}, key="fs_before")
        maybe_popup("fair_share_before", r_before, "fs_before")
        st.markdown(f'<div style="font-size:1.3rem;color:#CF3B30;font-weight:700;">{t("charts.gap", L)}: {original_gap_pp:.1f} pp</div>', unsafe_allow_html=True)
        st.markdown(f'<div style="font-size:0.8rem;color:#637381;">{"El gap sigue creciendo. La red colapsa." if L == "es" else "The gap keeps growing. The network collapses."}</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)

    with col_arrow:
        st.markdown(f"""
        <div style="display:flex;align-items:center;justify-content:center;height:100%;font-size:2.5rem;color:#D97724;">
          →
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_after:
        st.markdown('<div class="chart-container" style="text-align:center;">', unsafe_allow_html=True)
        st.markdown(f'<div style="font-size:0.9rem;color:#2EA043;font-weight:700;text-transform:uppercase;letter-spacing:1px;">✅ {t("fair_share.ott_scenario", L).format(filters["ott_pct"]*100)}</div>', unsafe_allow_html=True)
        remaining = max(impact["remaining_gap_pp"], 0)
        from streamlit_echarts import st_echarts
        r_after = st_echarts(options=_gauge_chart(remaining, gauge_max, "#2EA043", ""), height="220px", events={"click": CLICK_FORMAT}, key="fs_after")
        maybe_popup("fair_share_after", r_after, "fs_after")
        st.markdown(f'<div style="font-size:1.3rem;color:#2EA043;font-weight:700;">{t("fair_share.remaining_gap", L)}: {remaining:.1f} pp</div>', unsafe_allow_html=True)
        st.markdown(f'<div style="font-size:0.8rem;color:#637381;">{"Se cierran" if L == "es" else "Closes"} {impact["gap_closed_pp"]:.0f} pp</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    from streamlit_echarts import st_echarts
    donut_result = st_echarts(options=ott_donut_echarts(L), height="320px", events={"click": CLICK_FORMAT}, key="ott_donut")
    ...
    <div style="text-align:right;padding:0.5rem 1rem;margin-top:0.5rem;font-size:0.9rem;color:#8B949E;font-style:italic;border-top:1px solid rgba(255,255,255,0.04);">
      {t('fair_share.transition', L)} <span style="color:#2EA043;">→</span>
    </div>
    """, unsafe_allow_html=True)
