...
def _qr_img(url: str):
    ...
    qr.add_data(url)
    qr.make(fit=True)
    img = qr.make_image(fill_color="#ECEFF1", back_color="#090D1A")
    buf = BytesIO()
...
The project tests **6 hypotheses** on market concentration, network stress, and European asymmetry, and simulates the regulatory impact of **Fair Share**.""",
    ...
    },
    {
        ...
        "findings_list": {
            "es": [
                "**H1 ✅** El Efecto Tijera existe: tráfico +12.000%, ingresos planos",
                "**H2 ❌ REFUTADA:** La concentración NO es la causa — el HHI bajó y la brecha subió",
                "**H3 ✅** El problema es estructural del modelo TELCO, no de poder de mercado",
                "**H4 ✅** Europa invierte la mitad que EE.UU. (118€ vs 226€ per cápita)",
                "**H5 ✅** El ARPU europeo es ~3x menor que el de EE.UU.",
                "**H6 ✅** Fair Share cierra la brecha significativamente",
            ],
            "en": [
                "**H1 ✅** The Scissors Effect is real: traffic +12,000%, revenue flat",
                "**H2 ❌ REFUTED:** Concentration is NOT the cause — HHI dropped, gap widened",
                "**H3 ✅** The problem is structural to the TELCO model, not market power",
                "**H4 ✅** Europe invests half of what the US does (€118 vs €226 per capita)",
                "**H5 ✅** European ARPU is ~3x lower than the US",
                "**H6 ✅** Fair Share significantly closes the gap",
            ],
        },
    },
    {
...
def render():
    ...
    st.markdown(f"""
    <div class="story-chapter" style="border-color:rgba({int(c[1:3],16)},{int(c[3:5],16)},{int(c[5:7],16)},0.3);">
      <span class="chapter-badge" style="background:linear-gradient(135deg,{c},#00897B);">📋 About</span>
    ...
    for sec in _SECTIONS:
        st.markdown(f'<div class="chart-container" style="border-left:4px solid {sec["color"]};">', unsafe_allow_html=True)
        st.markdown(f'<div class="chart-title" style="color:{sec["color"]};">{sec["icon"]} {sec["title"][L]}</div>', unsafe_allow_html=True)
        ...
        if "facts" in sec:
            ...
            for ci, fact in enumerate(sec["facts"][L]):
                with cols[ci]:
                    st.markdown(f"""
                    <div style="background:rgba(0,191,165,0.06);border-radius:10px;padding:0.6rem;text-align:center;
                                border:1px solid rgba(0,191,165,0.12);font-size:0.85rem;color:var(--text-primary);">
                      {fact}
                    </div>
                    """, unsafe_allow_html=True)
        ...
        if "sources" in sec:
            ...
            st.markdown("""<tr style="border-bottom:1px solid rgba(255,255,255,0.06);">
              <th style="text-align:left;padding:0.5rem 0.5rem;color:#90A4AE;font-weight:600;">Fuente</th>
              <th style="text-align:left;padding:0.5rem 0.5rem;color:#90A4AE;font-weight:600;">Organismo</th>
              <th style="text-align:left;padding:0.5rem 0.5rem;color:#90A4AE;font-weight:600;">Dataset</th>
              <th style="text-align:right;padding:0.5rem 0.5rem;color:#90A4AE;font-weight:600;">Volumen</th>
            </tr>""", unsafe_allow_html=True)
            for src in sec["sources"]:
                st.markdown(f"""<tr style="border-bottom:1px solid rgba(255,255,255,0.03);">
                  <td style="padding:0.45rem 0.5rem;color:#00BFA5;font-weight:600;">{src[0]}</td>
        ...
        if "tools" in sec:
            ...
            for i, (tool, desc) in enumerate(sec["tools"]):
                with cols[i % 2]:
                    st.markdown(f"""
                    <div style="background:rgba(124,77,255,0.05);border-radius:8px;padding:0.5rem 0.8rem;margin:0.25rem 0;
        ...
        if "findings_list" in sec:
            for finding in sec["findings_list"][L]:
                st.markdown(f"""
                <div style="padding:0.4rem 0.8rem;margin:0.2rem 0;border-left:3px solid {sec["color"]};
                            background:rgba({int(sec["color"][1:3],16)},{int(sec["color"][3:5],16)},{int(sec["color"][5:7],16)},0.04);
        ...
        if "badges" in sec:
            ...
            for ci, badge in enumerate(sec["badges"][L]):
                with cols[ci]:
                    st.markdown(f"""
                    <div style="background:rgba(0,230,118,0.06);border-radius:20px;padding:0.3rem 0.6rem;text-align:center;
                                border:1px solid rgba(0,230,118,0.15);font-size:0.8rem;color:#00E676;">
                      {badge}
                    </div>
                    """, unsafe_allow_html=True)
    ...
    with col_link:
        ...
        <div style="height:100%;display:flex;flex-direction:column;justify-content:center;padding:1rem 2rem;">
          <div style="font-size:1.1rem;font-weight:600;color:var(--accent-teal);margin-bottom:0.5rem;">
            🔗 <a href="https://github.com/juandelaf1/NetTension" target="_blank">github.com/juandelaf1/NetTension</a>
          </div>
    ...
    <div style="text-align:center;padding:2rem 1rem 0.5rem;font-size:0.75rem;color:var(--text-muted);">
      NetTension v1.0 · CNMC + Eurostat · Junio 2026
    </div>
    """, unsafe_allow_html=True)
